...
**Part 14 - Justified Suppression and False Positive Feedback**
- `bawbel accept` CLI command - mark findings as false positives or accepted risks
- `bawbel-accept` inline comment syntax - accepted risk with expiry date
- Extended `bawbel-ignore` syntax - supports reason/reviewer/reviewed metadata
- Expiry enforcement - expired accepted risks resurface as active findings automatically
- `bawbel accept --list` - list all accepted findings with expiry status
- `bawbel accept --expiring-soon` - show findings expiring within N days (CI: exit 1 if within 14d)
- `accepted_findings` array in JSON output - full metadata for audit trail
- Anonymous FP signal reporting to PiranhaDB via `--report` flag
- `AcceptedFinding` dataclass in `scanner.models.acceptance`
- `scanner/justified_suppression.py` - parser and expiry enforcer
- `scanner/suppression.py` - base suppression engine (extracted from scanner.py)

**New AVE records and detection rules**
- AVE-2026-00046 `bawbel-hook-hijack` - MCP tool hook hijacking (CRITICAL, AIVSS 9.1)
- AVE-2026-00047 `bawbel-hardcoded-credential` - hardcoded credentials in agent components (HIGH, AIVSS 7.8)
- AVE-2026-00048 `bawbel-unsafe-delegation` - unsafe agent delegation chain (HIGH, AIVSS 8.2)
- `bawbel creds ./` command (`cmd_creds.py`) - credential-focused scan with dedicated output
- `bawbel chain ./` command (`cmd_chain.py`) - delegation chain mapper

**CI/CD**
- Fixed `pr-review.yml` regression-check job: added `pip install -e .` before scanner invocation

**ScanResult**
- `accepted_findings: list[AcceptedFinding]` field added (additive, non-breaking)
- `to_dict()` now includes `accepted_findings` array

### Changed
- `scanner.py` now runs Step 10: justified suppression after Step 9: inline suppression
- Pattern engine: 37 -> 40 rules (3 new AVE records)

### Fixed
- `pr-review.yml` regression-check failed on clean repos due to missing `pip install -e .`
...
[0.1.0]: https://github.com/bawbel/scanner/releases/tag/v0.1.0# Changelog
